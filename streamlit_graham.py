...
# Converter ROE e CAGR para % legível
if 'ROE (%)' in tabela_rename.columns:
    tabela_rename['ROE (%)'] = (tabela_rename['ROE (%)'] * 1).round(2)
if 'CAGR 5a (%)' in tabela_rename.columns:
    tabela_rename['CAGR 5a (%)'] = (tabela_rename['CAGR 5a (%)'] * 1).round(2)
...
def highlight_val(val):
    if isinstance(val, (int, float)):
        if val > 100:
            ...
            return 'background-color: #c1121f; color: white'
    return ''

format_dict = {col: "{:.2f}" for col in tabela_rename.select_dtypes(include='number').columns}
styled = tabela_rename.style.applymap(highlight_val, subset=['Valorização (%)']).format(format_dict)
